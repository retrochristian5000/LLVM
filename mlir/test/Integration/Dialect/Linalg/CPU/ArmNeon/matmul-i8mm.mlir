...
module attributes {transform.with_named_sequence} {
  ...
  transform.named_sequence @tile_and_vectorize_matmul(%func
    ...
    // Step 0: Get a handle to the matmul op, if any (functions without one,
    // like the `printMemrefI32` declaration below, are simply unaffected).
    ...
    // Step 2: Vectorize directly to a named `vector.contract`, reusing the
    // matmul's own indexing maps (RHS "N-major") instead of the maps a
    // generic vectorize + reduction_to_contract would infer.
    ...
    // (`arm_neon.intr.smmla`/`ummla`/`usmmla`), instead of the generic
    // outerproduct lowering used by the plain-f32 NEON matmul test, which
    // would never exercise `smmla`.
    transform.apply_patterns to %func {
