...
//===----------------------------------------------------------------------===//
//
// Example plugin showing how an out-of-tree MLIR pass can be added to flang's
// HLFIR-to-FIR pass pipeline, while the HLFIR intrinsic operations (hlfir.sum,
// hlfir.matmul, ...) are still present. It contributes a pass that prints those
// operations, tagged with the pipeline position it was inserted at.
//
// The pass is exposed through both plugin entry points. For `flang -fc1 -load`
// a static initializer calls fir::registerPassPipelineConfigCallback to reach
// the config the frontend builds, and registers the pass at the HLFIROptEarly
// and HLFIROptLast extension points. For fir-opt, mlirGetPassPluginInfo makes
// it available to --load-pass-plugin:
//
...
/// caller-supplied label identifying the pipeline position. Reports on the
/// whole `hlfir` dialect rather than a hard-coded op list, so that the example
/// does not need to link the HLFIR dialect library.
struct PrintHLFIRIntrinsicsPass
  ...
  Option<std::string> label{*this, "label",
      llvm::cl::desc("Tag printed with every line, identifying the pipeline "
                     "position this pass was inserted at"),
      llvm::cl::init("hlfir")};
...
/// Registers the pass at both HLFIR extension points.
...
// Runs when the shared object is dlopen'd, before any compilation starts.
