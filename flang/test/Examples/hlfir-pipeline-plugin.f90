! Check that a `flang -fc1 -load`'ed plugin can insert MLIR passes into the
! HLFIR-to-FIR pass pipeline through the HLFIR extension points.
! This requires that the examples are built (LLVM_INCLUDE_EXAMPLES=ON) to access
! flangHLFIRPipelinePlugin.so
...
! At -O2 SimplifyHLFIRIntrinsics has expanded the intrinsics before the Last
