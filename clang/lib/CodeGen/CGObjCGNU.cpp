...
protected:
  ...
  std::string SymbolForProtocolRef(StringRef Name) {
    return llvm::Twine(ManglePublicSymbol("OBJC_REF_PROTOCOL_"))
        .concat(Name)
        .str();
  }
...
class CGObjCGNUstep2 : public CGObjCGNUstep {
  ...
  std::string SymbolForClassRef(StringRef Name, bool isWeak) {
    if (isWeak)
      return llvm::Twine(ManglePublicSymbol("OBJC_WEAK_REF_CLASS_"))
          .concat(Name)
          .str();
    else
      return llvm::Twine(ManglePublicSymbol("OBJC_REF_CLASS_"))
          .concat(Name)
          .str();
  }
  /// Generate the name of a class symbol.
  std::string SymbolForClass(StringRef Name) {
    return llvm::Twine(ManglePublicSymbol("OBJC_CLASS_"))
        .concat(Name)
        .str();
  }
