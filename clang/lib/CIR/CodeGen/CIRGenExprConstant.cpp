...
static bool updateRecord(ConstantEmitter &emitter, CIRGenModule &cgm,
  ...
  for (const FieldDecl *field : rd->fields()) {
    ...
    if (fieldNo >= updater->getNumInits())
      break;

    const Expr *init = updater->getInit(fieldNo);
    ++fieldNo;
...
static bool updateArray(ConstantEmitter &emitter, CIRGenModule &cgm,
  ...
  // Decompose the base array into mutable elements.
  llvm::SmallVector<mlir::Attribute> elements;
  ...
  if (auto arrAttr = mlir::dyn_cast<cir::ConstArrayAttr>(base)) {
    ...
    for (unsigned i = 0; i < eltsAttr.size(); ++i)
      elements[i] = eltsAttr[i];
    // Fill trailing zeros with zero attrs.
    mlir::Type eltTy = cgm.convertType(elemType);
    for (uint64_t i = eltsAttr.size(); i < numElements; ++i)
      elements[i] = builder.getZeroInitAttr(eltTy);
  } else if (mlir::isa<cir::ZeroAttr>(base)) {
    mlir::Type eltTy = cgm.convertType(elemType);
  ...
  // Rebuild the array attr, trimming trailing zeros.
