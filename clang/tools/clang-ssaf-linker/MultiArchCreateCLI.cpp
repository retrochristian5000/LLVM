...
void MultiArchCreateCLI::run(llvm::TimerGroup &TG, const Config &Cfg) {
  info(Cfg.Verbose, 0, "Creating multi-arch bundle started.");

  {
    info(Cfg.Verbose, 1, "Validating input.");
    validate(TG, Cfg);
  }

  info(Cfg.Verbose, 1, "Bundling members.");
  bundle(TG, Cfg);
  write(TG, Cfg);

  info(Cfg.Verbose, 0, "Creating multi-arch bundle finished.");
}

void MultiArchCreateCLI::validate(llvm::TimerGroup &TG, const Config &Cfg) {
  ...
  info(Cfg.Verbose, 2, "Validated output path '{0}'.", OutputFile.Path);
  ...
  info(Cfg.Verbose, 2, "Validated {0} input artifact paths.",
       InputFiles.size());
}

void MultiArchCreateCLI::bundle(llvm::TimerGroup &TG, const Config &Cfg) {
  ...
  info(Cfg.Verbose, 2, "Classifying inputs.");

  for (auto [Index, InputFile] : llvm::enumerate(InputFiles)) {
    info(Cfg.Verbose, 3, "[{0}/{1}] Reading '{2}'.", (Index + 1),
         InputFiles.size(), InputFile.Path);
    ...
    info(Cfg.Verbose, 3, "[{0}/{1}] Classifying '{2}'.", (Index + 1),
         InputFiles.size(), InputFile.Path);
  ...
  if (ResultFamily == Family::Static) {
    ...
    BuildNamespace ExpectedWrapper(BuildNamespaceKind::MultiArchStaticLibrary,
                                   TargetName);
    info(Cfg.Verbose, 2, "Target namespace: '{0}'.", ExpectedWrapper);
    ...
    info(Cfg.Verbose, 2, "Bundled {0} architecture slice(s).",
         StaticResult->Members.size());
  } else {
    ...
    info(Cfg.Verbose, 2, "Target namespace: '{0}'.", TargetNamespace);
    ...
    info(Cfg.Verbose, 2, "Bundled {0} architecture slice(s).",
         SharedResult->Members.size());
  }
}

void MultiArchCreateCLI::write(llvm::TimerGroup &TG, const Config &Cfg) {
  info(Cfg.Verbose, 2, "Writing multi-arch bundle to '{0}'.", OutputFile.Path);
