...
// RUN: %clang -target x86_64-linux-android -### %s 2>&1 | FileCheck %s --check-prefix=ENABLED
// RUN: %clang -target i686-linux-android -### %s 2>&1 | FileCheck %s --check-prefix=ENABLED
// RUN: %clang -target aarch64-linux-android -### %s 2>&1 | FileCheck %s --check-prefix=ENABLED
// RUN: %clang -target riscv64-linux-android -### %s 2>&1 | FileCheck %s --check-prefix=ENABLED
// RUN: %clang -target aarch64-linux-android -fno-stack-clash-protection -### %s 2>&1 | FileCheck %s --check-prefix=DISABLED --implicit-check-not='"-fstack-clash-protection"'
// RUN: %clang -target aarch64-linux-android -fno-stack-clash-protection -fstack-clash-protection -### %s 2>&1 | FileCheck %s --check-prefix=ENABLED
// RUN: %clang -target aarch64-linux-android -fstack-clash-protection -fno-stack-clash-protection -### %s 2>&1 | FileCheck %s --check-prefix=DISABLED --implicit-check-not='"-fstack-clash-protection"'
