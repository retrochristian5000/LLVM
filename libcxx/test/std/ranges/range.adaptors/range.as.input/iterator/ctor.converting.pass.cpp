...
constexpr bool test() {
  ...
  using ConstIterator = std::ranges::iterator_t<const decltype(view)>;

  static_assert(!std::same_as<Iterator, ConstIterator>);
  static_assert(std::convertible_to<Iterator, ConstIterator>);
  static_assert(std::constructible_from<ConstIterator, Iterator&&>);

  Iterator it = view.begin();
  ConstIterator const_it{std::move(it)};
