...
constexpr bool test() {
  { // base() &&
    ...
    using Iterator = std::ranges::iterator_t<CopyConstructibleView>;

    std::same_as<Iterator> decltype(auto) base_it = std::move(it).base();
  ...
  { // base() const & noexcept
    {
      ...
      { // &
        std::same_as<const Iterator&> decltype(auto) base_it = it.base();
      ...
      { // const &
        std::same_as<const Iterator&> decltype(auto) base_it = std::as_const(it).base();
        static_assert(noexcept(it.base()));
      ...
      { // const &&
        std::same_as<const Iterator&> decltype(auto) base_it = std::move(std::as_const(it)).base();
        static_assert(noexcept(it.base()));
    ...
    }
    {
      ...
      using Iterator = std::ranges::iterator_t<NonCopyConstructibleView>;
