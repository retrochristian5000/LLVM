...
template <class... _Args>
_LIBCPP_NO_SPECIALIZATIONS inline const bool __is_invocable_v = __is_invocable_impl<void, _Args...>;
...
template <class... _Args>
inline const bool
    __is_nothrow_invocable_impl<__enable_if_t<noexcept(__builtin_invoke(std::declval<_Args>()...))>, _Args...> = true;
#  endif

template <class... _Args>
_LIBCPP_NO_SPECIALIZATIONS inline const bool __is_nothrow_invocable_v = __is_nothrow_invocable_impl<void, _Args...>;
...
using __invoker_t _LIBCPP_NODEBUG = _Invoker<
    ...
    (!__conditional_t<
         _IsRefWrapper::value,
         false_type,
         __conditional_t<is_same<_Class, _DecayA0>::value, is_same<_Class, _DecayA0>, is_base_of<_Class, _DecayA0> > >::
         value
     << 1) +
    !_IsRefWrapper::value>;
...
struct _Invoker<1> {
  ...
  _Fn(_Fp __f, _A0&& __a0, _Args&&... __args)
      _NOEXCEPT_(noexcept((std::declval<_A0>().*std::declval<_Fp>())(std::declval<_Args>()...))) {
...
struct _Invoker<4> {
  ...
  _Fn(_Fp __f, _A0&& __a0) _NOEXCEPT_(noexcept(std::declval<_A0>().*std::declval<_Fp>())) {
...
struct _Invoker<5> {
  template <class _Fp, class _A0>
  static inline _LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR decltype(std::declval<_A0&>().*std::declval<_Fp>())
  _Fn(_Fp __f, reference_wrapper<_A0> __a0) _NOEXCEPT_(noexcept(std::declval<_A0&>().*std::declval<_Fp>())) {
    return __a0.get().*__f;
  }
};
...
__invoke(_Fp&& __f, _Args&&... __args) _NOEXCEPT_(noexcept(std::declval<_Fp>()(std::declval<_Args>()...))) {
...
__invoke(_Member _Class::* __f, _A0&& __a0, _Args&&... __args)
    _NOEXCEPT_(noexcept(std::__invoker_t<_Member, _Class, _A0>::_Fn(
        std::declval<_Member _Class::*>(), std::declval<_A0>(), std::declval<_Args>()...))) {
  return std::__invoker_t<_Member, _Class, _A0>::_Fn(__f, static_cast<_A0&&>(__a0), static_cast<_Args&&>(__args)...);
}

template <class... _Args>
using __invoke_result_t _LIBCPP_NODEBUG = decltype(std::__invoke(std::declval<_Args>()...));
...
inline const bool __is_invocable_r_impl<decltype(void(std::__invoke(std::declval<_Fp>(), std::declval<_Args>()...))),
                                        ...
                                        _Fp,
                                        _Args...> =
    is_void<_Ret>::value || __is_core_convertible<__invoke_result_t<_Fp, _Args...>, _Ret>::value;

template <class _Ret, class... _Args>
_LIBCPP_NO_SPECIALIZATIONS inline const bool __is_invocable_r_v = __is_invocable_r_impl<void, _Ret, _Args...>;
...
template <class _Fp, class... _Args>
inline const bool
    __is_nothrow_invocable_impl<__enable_if_t<noexcept(std::__invoke(std::declval<_Fp>(), std::declval<_Args>()...))>,
                                _Fp,
                                _Args...> = true;
...
template <class _Void, class _Ret, class... _Args>
inline const bool __nothrow_invokable_r_imp = false;

template <class _Ret, class _Fp, class... _Args>
inline const bool __nothrow_invokable_r_imp<
...
template <class... _Args>
inline const bool __nothrow_invokable_r_imp<__enable_if_t<__is_nothrow_invocable_v<_Args...> >, void, _Args...> = true;

template <class _Ret, class... _Args>
using __is_nothrow_invocable_r _LIBCPP_NODEBUG =
    _BoolConstant<__nothrow_invokable_r_imp<void, __conditional_t<is_void<_Ret>::value, void, _Ret>, _Args...> >;
...
template <class _Ret, class... _Args>
using __is_invocable_r _LIBCPP_NODEBUG = _BoolConstant<__is_invocable_r_v<_Ret, _Args...> >;

template <class _Ret, class... _Args>
_LIBCPP_HIDE_FROM_ABI _LIBCPP_CONSTEXPR_SINCE_CXX20 decltype((void)std::__invoke(
    std::declval<__enable_if_t<is_void<_Ret>::value, _Args> >()...))
__invoke_r(_Args&&... __args) _NOEXCEPT_(noexcept(std::__invoke(std::declval<_Args>()...))) {
  std::__invoke(static_cast<_Args&&>(__args)...);
}

template <class _Ret, class... _Args>
_LIBCPP_HIDE_FROM_ABI
_LIBCPP_CONSTEXPR_SINCE_CXX20 decltype(std::__invoke(std::declval<__enable_if_t<!is_void<_Ret>::value, _Args> >()...))
__invoke_r(_Args&&... __args) _NOEXCEPT_(noexcept(std::__invoke(std::declval<_Args>()...))) {
