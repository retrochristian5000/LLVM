...
bool passesTidyLineFilter(const tidy::ClangTidyOptions &Options,
  ...
  if (!Options.LineFilter || Options.LineFilter->empty())
    return true;
  ...
  unsigned LineNumber = SM.getExpansionLineNumber(Loc);
  for (const tidy::FileFilter &Filter : *Options.LineFilter) {
    if (!FileName.ends_with(Filter.Name))
      continue;
    if (Filter.LineRanges.empty())
      return true;
    return llvm::any_of(
        Filter.LineRanges, [&](const tidy::FileFilter::LineRange &Range) {
