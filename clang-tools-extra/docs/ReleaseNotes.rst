...
- Improved :doc:`bugprone-narrowing-conversions
  <clang-tidy/checks/bugprone/narrowing-conversions>` check by:

  - fixing a false positive when converting a ``bool`` to a signed integer type.

  - adding the :option:`WarnOnTimeTNarrowingConversion` option to diagnose
    conversions from ``time_t`` values to integer types that may not preserve
    the full range of ``time_t``. This option is disabled by default.
