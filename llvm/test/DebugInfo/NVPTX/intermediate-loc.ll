...
;; The intermediate DIFile has no source, so its secondary .file uses the
;; filename (the checksum-named form is used only when source is present).
; CHECK: .file [[INTFILE]] ".{{/|\\\\}}intermediate.ptx"
...
!14 = !DIFile(filename: "intermediate.ptx", directory: ".", checksumkind: CSK_MD5, checksum: "0123456789abcdef0123456789abcdef")
