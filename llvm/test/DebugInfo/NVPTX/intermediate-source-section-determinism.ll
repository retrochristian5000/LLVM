...
;; deterministic order: first-reference (insertion) order, which is also
;; ascending .file-number order. The test pins that the ordering key is
;; reference/file-number order, not the filename -- "bbb.tileir" is referenced
;; first (lower .file number) and "aaa.tileir" second, so bbb's code_block is
;; emitted first even though "aaa" sorts earlier alphabetically.
...
; CHECK-NEXT:   .source: <<< bbb source line >>>
