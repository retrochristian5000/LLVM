...
; RUN: llvm-extract -S -func=multiple_BBs_in_loop %s -o %t.multibb
; RUN: opt < %t.multibb -passes=loop-interchange -cache-line-size=64 \
; RUN:     -loop-interchange-profitabilities=vectorize \
; RUN:     -pass-remarks-output=%t.multibb.yaml -disable-output
; RUN: FileCheck %s --check-prefix=MULTIBB --input-file=%t.multibb.yaml \
; RUN:     --implicit-check-not=UnsafeInst
...
; MULTIBB:      --- !Missed
; MULTIBB:      Name:            InterchangeNotProfitable
; MULTIBB-NEXT: Function:        multiple_BBs_in_loop
