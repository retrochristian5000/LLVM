...
entry:
  ...
  %s0 = call <32 x i32> asm sideeffect "", "=s" ()
  %s1 = call <32 x i32> asm sideeffect "", "=s" ()
  %s2 = call <32 x i32> asm sideeffect "", "=s" ()

  %cmp = icmp eq i32 %in, 0
  br i1 %cmp, label %use, label %exit

use:
  call void asm sideeffect "", "s"(<32 x i32> %s0)
  call void asm sideeffect "", "s"(<32 x i32> %s1)
  call void asm sideeffect "", "s"(<32 x i32> %s2)
  br label %exit
