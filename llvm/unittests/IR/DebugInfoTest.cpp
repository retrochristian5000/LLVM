...
// Under the outermost-frame model, two instructions inlined into the same kernel
// share the kernel (outer) frame that carries the tile-IR snapshot, while their
// heads (inner frames) differ. Merging them must PRESERVE that shared outer-frame
// layer (MergeLocPair threads per-frame layers), so the merged instruction still
// resolves to the kernel's tile-IR line.
TEST(DebugLocTest, MergedLocationOuterFrameLayerPreserved) {
  ...
  // Two instructions inlined into that kernel frame: same callee, different head
  // lines, no layer of their own.
  ...
  // ...but the shared outer (kernel) frame -- and its snapshot layer -- survives,
  // so the emission walk still resolves to the kernel's tile-IR line.
