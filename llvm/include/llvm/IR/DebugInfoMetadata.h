...
public:
  ...
  MDString *getRawKind() const { return cast_or_null<MDString>(getOperand(0)); }
...
/// An ordered, uniqued list of \a DILayerLoc entries.
///
...
/// own metadata kind (so `isa<DILayerLocList>` works, which the DILocation
/// operand disambiguation relies on).
...
public:
  ...
  DILayerLoc *getLayer(unsigned I) const {
    return cast_or_null<DILayerLoc>(getOperand(I));
  }
...
class DILocation : public MDNode {
  ...
  uint64_t AtomRank : 3;
  // Disambiguates the two optional trailing operands: with both inlinedAt and
  // irlayers optional, operand count alone is ambiguous. irlayers, when
  // present, is always the last operand. Layout: [scope, (inlinedAt?),
  // (irlayers?)].
...
public:
  ...
  DILayerLocList *getIRLayers() const {
    return HasIRLayers ? cast<DILayerLocList>(getOperand(getNumOperands() - 1))
                       : nullptr;
  }
  unsigned getNumLayers() const {
    return HasIRLayers ? getIRLayers()->getNumLayers() : 0;
  }
  DILayerLoc *getLayer(unsigned I) const {
    return HasIRLayers ? getIRLayers()->getLayer(I) : nullptr;
  }
