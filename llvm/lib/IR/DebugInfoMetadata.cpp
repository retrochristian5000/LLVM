...
DILayerLoc *DILayerLoc::getImpl(LLVMContext &Context, MDString *Kind,
  ...
  // Clamp an out-of-range column to 0 (the 16-bit SubclassData limit), matching
  // DILocation::getImpl; the ctor otherwise asserts and release builds truncate.
  adjustColumn(Column);
  if (Storage == Uniqued) {
...
// Returns a uniqued DILayerLocList holding the intersection (by DILayerLoc
// pointer identity) of LocA's and LocB's layer sets, or null if either has no
// layers or they share none.
static Metadata *mergeIRLayers(LLVMContext &C, const DILocation *LocA,
  ...
  if (!LA || !LB)
    return nullptr;
  // Keep LA's entries (in LA's order) that also appear in LB. DILayerLocs are
  // uniqued, so pointer identity is structural identity.
  SmallPtrSet<Metadata *, 4> BLayers;
  for (const MDOperand &Op : LB->operands())
    BLayers.insert(Op.get());
  SmallVector<Metadata *, 2> Keep;
  for (const MDOperand &Op : LA->operands())
    if (BLayers.contains(Op.get()))
      Keep.push_back(Op.get());
  if (Keep.empty())
    return nullptr;
    ...
    if (Scope->getFile() != L1->getFile() || L1->getFile() != L2->getFile()) {
      ...
      if (Scope->getFile() != L1->getFile() || L1->getFile() != L2->getFile())
        return DILocation::get(C, CommonLoc.first, CommonLoc.second,
                               CommonLocScope, InlinedAt, /*ImplicitCode=*/false,
