...
size_t ProcessWasm::ReadGlobal(uint32_t module_id, uint32_t index, void *buf,
  ...
  const uint32_t frame_index = CanQueryInstance(module_id)
...
ProcessWasm::GetWasmGlobal(uint32_t module_id, uint32_t index,
  ...
  // A frame stands in for the module it is executing only where the stub cannot
...
bool ProcessWasm::CanQueryInstance(uint32_t module_id) {
