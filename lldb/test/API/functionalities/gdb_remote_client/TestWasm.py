...
# The key under which a packet names the instance whose state it reads. An
# instance is an address space of the process, which is what the key names.
ADDRESS_SPACE_KEY = "address_space:"
...
def global_read_packet(global_index, module_id=MODULE_ID):
    """
    The packet that reads a global from the address space of the instance
    holding it.
    """
    return f"qWasmGlobal:{global_index};{ADDRESS_SPACE_KEY}{module_id};"
...
class MyResponder(MockGDBServerResponder):
    ...
    def qWasmGlobal(self, packet):
        ...
        with the address space suffix, and one that cannot names a frame.

        Format: qWasmGlobal:index;address_space:id; or
                qWasmGlobal:frame_index;index
        """
        first, _, rest = packet.split(":", 1)[1].partition(";")

        if rest.startswith(ADDRESS_SPACE_KEY):
            ...
            module = self.module_with_id(
                int(rest[len(ADDRESS_SPACE_KEY) :].rstrip(";"))
            )
            if module is None:
                return "E04"
...
class TestWasm(GDBRemoteTestBase):
    ...
    def global_reads_received(self, address_space):
        """
        The global reads the client sent that name an address space, or those
        that name a frame instead. Both forms share a packet name, so the suffix
        is what tells them apart.
        """
        return [
            packet
            for packet in self.packets_received("qWasmGlobal:")
            if (ADDRESS_SPACE_KEY in packet) == address_space
        ]
    ...
    def test_read_global(self):
        ...
        self.assertEqual([], self.global_reads_received(address_space=False))
    ...
    def test_read_global_without_reported_call_stack(self):
        ...
        # something else, and is what a stub that cannot be given an address
        # space is limited to.
    ...
    def test_read_variable_located_through_global_legacy_server(self):
        ...
        self.assertEqual([], self.global_reads_received(address_space=True))
